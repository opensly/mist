...
export class EditorFormattingService {
  ...
  insertBlock(editor: HTMLElement, html: string): void {
    ...
    // Check if this is trusted HTML (panel or table) generated by the app
    const isTrustedContent = html.includes('editor-panel') || html.includes('<table');
    
    // Sanitize HTML before insertion
    // Use trusted sanitization for app-generated content (panels, tables), regular for user content
    const sanitizedHtml = isTrustedContent 
